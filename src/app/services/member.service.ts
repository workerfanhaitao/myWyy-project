import { Signin, UserRecord, UserSheet, RecordVal } from './data-types/member.type';
...
export class MemberService {
  ...
  signin(): Observable<Signin> {
    ...
    .pipe(map((result) => result as Signin));
  }
  ...
  // 收藏歌曲
  likeSongToSheet({ pid, tracks }: LikeSongParams): Observable<number> {
  ...
  // 创建歌单
  createSheet(sheetName: string): Observable<number> {
    const params = new HttpParams({ fromString: queryString.stringify({name: sheetName}) });
    return this.http.get(this.uri + 'playlist/create', { params }).pipe(map((res: SampleBack) => res.id));
  }

  // 收藏歌单
  likeSheet(id: number, t = 1): Observable<number> {
    const params = new HttpParams({ fromString: queryString.stringify({ t, id}) });
  ...
  // 发送验证码
  sentCode(phone: number): Observable<number> {
